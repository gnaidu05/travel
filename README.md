...
2. Run the SQL files in `supabase/migrations` in numerical order in the Supabase SQL editor.
...
All application tables use Row Level Security. Authenticated accounts whose email ends in `@capgemini.com` can view shared planning data. Only approved administrators can edit. View-only users can request administrator access, which an existing administrator can approve or reject in the application. Anonymous visitors cannot read the roster or employee data.
